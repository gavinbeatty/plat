...
macro(cmf_version_major_number _a _a_major)
    string(REGEX REPLACE "^([0-9]+)" "\\1" "${_a_major}" "${_a}")
    set("${_a_major}" "${CMAKE_MATCH_1}")
    if("${${_a_major}}" STREQUAL "")
        set("${_a_major}" "0")
    endif("${${_a_major}}" STREQUAL "")
endmacro(cmf_version_major_number)
macro(cmf_version_minor_number _a _a_minor)
    string(REGEX REPLACE "^[0-9]+\\.([0-9]+)" "\\1" "${_a_minor}" "${_a}")
    set("${_a_minor}" "${CMAKE_MATCH_1}")
    if("${${_a_minor}}" STREQUAL "")
        set("${_a_minor}" "0")
    endif("${${_a_minor}}" STREQUAL "")
endmacro(cmf_version_minor_number)
macro(cmf_version_patch_number _a _a_patch)
    string(REGEX REPLACE "^[0-9]+\\.[0-9]+\\.([0-9]+)" "\\1" "${_a_patch}" "${_a}")
    set("${_a_patch}" "${CMAKE_MATCH_1}")
    if("${${_a_patch}}" STREQUAL "")
        set("${_a_patch}" "0")
    endif("${${_a_patch}}" STREQUAL "")
endmacro(cmf_version_patch_number)
macro(cmf_version_other_number _a _a_other)
    string(REGEX REPLACE "^[0-9]+\\.[0-9]+\\.[0-9]+\\.([0-9]+)" "\\1" "${_a_other}" "${_a}")
    set("${_a_other}" "${CMAKE_MATCH_1}")
    if("${${_a_other}}" STREQUAL "")
        set("${_a_other}" "0")
    endif("${${_a_other}}" STREQUAL "")
endmacro(cmf_version_other_number)
macro(cmf_version_letter _a _a_letter)
    string(REGEX REPLACE "^[0-9.]*([a-zA-Z])$" "\\1" "${_a_letter}" "${_a}")
    set("${_a_letter}" "${CMAKE_MATCH_1}")
endmacro(cmf_version_letter)
function(cmf_normalize_version _a _norm _letter)
    cmf_version_major_number("${_a}" major)
    cmf_version_minor_number("${_a}" minor)
    cmf_version_patch_number("${_a}" patch)
    cmf_version_other_number("${_a}" other)
    cmf_version_letter("${_a}" "${_letter}")
    foreach(i "major" "minor" "patch" "other")
        if("${${i}}" GREATER 99)
            message(SEND_ERROR "cmf_normalize_version: can only handle version numbers less than 100 in each position.")
        endif("${${i}}" GREATER 99)
    endforeach(i "major" "minor" "patch" "other")
    math(EXPR "${_norm}" "${other} + ( ${patch} * 100 ) + ( ${minor} * 10000 ) + ( ${major} * 1000000 )")
    set("${_norm}" "${${_norm}}" PARENT_SCOPE)
    set("${_letter}" "${${_letter}}" PARENT_SCOPE)
endfunction(cmf_normalize_version)

macro(cmf_version_letter_cmp_ _a _b _result)
    if("${_a}" STREQUAL "${_b}")
        set("${_result}" 0)
    elseif("${_a}" STREQUAL "")
        set("${_result}" -1)
...
endmacro(cmf_version_letter_cmp_)

macro(cmf_cmp_op_result_match _op _cmpresult _result)
    set("${_result}" 0)
...
endmacro(cmf_cmp_op_result_match)

macro(cmf_version_cmp _a _b _result)
    if("${_a}" STREQUAL "${_b}")
        set("${_result}" 0)
    else("${_a}" STREQUAL "${_b}")
        cmf_normalize_version("${_a}" _a_norm _a_letter)
        cmf_normalize_version("${_b}" _b_norm _b_letter)
        if("${_a_norm}" EQUAL "${_b_norm}")
            cmf_version_letter_cmp_("${_a_letter}" "${_b_letter}" "${_result}")
...
macro(m_check_version _proj _depname _foundvar)
    ...
    cmf_version_cmp("${${_depname}_ver}" "${${_depname}_find_ver}" _cmp)
...
macro(m_req_dep _name _depname _exactornot _ver _foundvar)
    cmf_version_major_number("${_ver}" _major)
    cmf_version_minor_number("${_ver}" _minor)
    cmf_version_patch_number("${_ver}" _patch)
    set(${_foundvar} TRUE)
